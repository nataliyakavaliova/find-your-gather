...
function ModerationPage() {
  ...
  const { data: reports, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data: profs } = reporterIds.length ? await supabase.from("profiles").select("id, full_name").in("id", reporterIds) : { data: [] as { id: string; full_name: string | null }[] };
      ...
      const { data: photos } = photoIds.length ? await supabase.from("gallery_photos").select("id, event_id, caption").in("id", photoIds) : { data: [] };
      ...
      return rows.map((r) => ({
        ...
        label: r.target_type === "event" ? (eventMap.get(r.target_id)?.title ?? "Event") : `Photo: ${photoMap.get(r.target_id)?.caption ?? "(no caption)"}`,
        event_id: r.target_type === "event" ? r.target_id : photoMap.get(r.target_id)?.event_id ?? "",
      }));
    },
  });
